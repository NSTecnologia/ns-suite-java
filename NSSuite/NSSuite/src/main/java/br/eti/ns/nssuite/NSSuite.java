...
import br.eti.ns.nssuite.requisicoes.ddfe.*;
...
import br.eti.ns.nssuite.retornos.ddfe.DownloadLoteRetDDFe;
...
public class NSSuite {
    ...
    // Metodos especificos DDFe
    public static String manifestacao(String caminho, String CNPJInteressado, String tpEvento, String tpAmb, String nsu, String chave, String xJust) throws IOException {
        ManifestacaoReqDDFe manifestacaoReqDDFe = new ManifestacaoReqDDFe();
        manifestacaoReqDDFe.CNPJInteressado = CNPJInteressado;
        manifestacaoReqDDFe.tpAmb = tpAmb;
        manifestacaoReqDDFe.manifestacao.tpEvento = tpEvento;
        if (nsu.equals("")){
            manifestacaoReqDDFe.chave = chave;
        }else{
            manifestacaoReqDDFe.nsu = nsu;
        }
        // Quando o evento for 210240, é obrigatoria ter um xJust
        if (tpEvento.equals("210240")){
            manifestacaoReqDDFe.manifestacao.xJust = xJust;
        }
        String url = endpoints.DDFeManifestacao;

        String json = objectMapper.writeValueAsString(manifestacaoReqDDFe);
        Genericos.gravarLinhaLog("ddfe", "[MANIFESTAÇÃO_DADOS]");
        Genericos.gravarLinhaLog("ddfe", json);

        String resposta = enviaConteudoParaAPI(json, url, "json");

        Genericos.gravarLinhaLog("ddfe", "[[MANIFESTAÇÃO_RESPOSTA]");
        Genericos.gravarLinhaLog("ddfe", resposta);

        tratamentoManifestacao(resposta, tpEvento, chave, caminho);
        return resposta;
    }

    public static void tratamentoManifestacao(String jsonRetorno, String tpEvento, String chave, String caminho) throws IOException {
        String xMotivo;
        JsonNode respostaJSON = objectMapper.readTree(jsonRetorno);
        String status = respostaJSON.get("status").asText();

        if (status.equals("200")){
            salvarDocManifestacao(jsonRetorno, tpEvento, chave, caminho);
    ...
    public static void salvarDocManifestacao(String jsonRetorno, String tpEvento, String chave, String caminho) throws IOException {
        JsonNode respostaJSON = objectMapper.readTree(jsonRetorno);
        String xml =  respostaJSON.get("retEvento").get("xml").asText();
        if(!caminho.endsWith("\\")) caminho += "\\";
        Genericos.salvarXML(xml, caminho, chave, tpEvento, "");
    }

    public static String downloadUnico(String caminho, String CNPJInteressado, String tpAmb, String modelo, String nsu, String chave, boolean incluirPdf, boolean apenasComXml, boolean comEventos) throws IOException {
        DownloadUnicoReqDDFe downloadUnicoReqDDFe = new DownloadUnicoReqDDFe();
        downloadUnicoReqDDFe.CNPJInteressado = CNPJInteressado;
        downloadUnicoReqDDFe.tpAmb = tpAmb;
        downloadUnicoReqDDFe.incluirPDF = incluirPdf;
        if (nsu.equals("")){
            downloadUnicoReqDDFe.chave = chave;
            downloadUnicoReqDDFe.apenasComXml = apenasComXml;
            downloadUnicoReqDDFe.comEventos = comEventos;
        }else{
            downloadUnicoReqDDFe.nsu = nsu;
            downloadUnicoReqDDFe.modelo = modelo;
        }

        String url = endpoints.DDFeDownloadUnico;
        String json = objectMapper.writeValueAsString(downloadUnicoReqDDFe);
        Genericos.gravarLinhaLog("ddfe", "[DOWNLOAD_UNICO_DADOS]");
        Genericos.gravarLinhaLog("ddfe", json);

        String resposta = enviaConteudoParaAPI(json, url, "json");

        Genericos.gravarLinhaLog("ddfe", "[[DOWNLOAD_UNICO_RESPOSTA]");
        Genericos.gravarLinhaLog("ddfe", resposta);

        tratamentoDownloadUnico(resposta, incluirPdf, caminho);

        return resposta;
    }

    public static void tratamentoDownloadUnico(String jsonRetorno, boolean incluirPdf, String caminho) throws IOException {
        JsonNode respostaJSON = objectMapper.readTree(jsonRetorno);
        String status = respostaJSON.get("status").asText();

        if(status.equals("200")){
            salvarDocUnico(jsonRetorno, incluirPdf, caminho);
            JOptionPane.showMessageDialog(null, "Download Unico feito com sucesso");
        }else{
            JOptionPane.showMessageDialog(null, respostaJSON.get("motivo").asText());
        }
    }

    public static void salvarDocUnico(String jsonRetorno, boolean incluirPdf, String caminho) throws IOException {
        JsonNode respostaJSON = objectMapper.readTree(jsonRetorno);
        String xml, chave, pdf;
        if(!caminho.endsWith("\\")) caminho += "\\";

        boolean listarDocs = respostaJSON.get("listaDocs").asBoolean();
        if (!listarDocs){
            ...
            if(incluirPdf){
                ...
                Genericos.salvarPDF(pdf, caminho, chave, "", "");
            }
        }else{
            String tpEvento = "";
            respostaJSON = respostaJSON.get("xmls");
            for (int i = 0; i<respostaJSON.size(); i++){
                if (i>=1){
                    tpEvento = respostaJSON.get(i).get("tpEvento").asText();
                }
                xml = respostaJSON.get(i).get("xml").asText();
                chave = respostaJSON.get(i).get("chave").asText();

                if(!xml.equals("")){
                    if((incluirPdf) && (tpEvento.equals(""))){
                        pdf = respostaJSON.get(i).get("pdf").asText();
                        Genericos.salvarPDF(pdf, caminho, chave, tpEvento, "");
                        tpEvento = "";
                    }
                    Genericos.salvarXML(xml, caminho, chave, tpEvento, "");
                }
    ...
    public static String downloadLote(String caminho, String CNPJInteressado, String tpAmb, String modelo, int ultNSU, boolean incluirPdf, boolean apenasComXml, boolean comEventos, boolean apenasPendManif, boolean retornoSimples) throws IOException {
        DownloadLoteReqDDFe downloadLoteReqDDFe = new DownloadLoteReqDDFe();
        downloadLoteReqDDFe.CNPJInteressado = CNPJInteressado;
        downloadLoteReqDDFe.tpAmb = tpAmb;
        downloadLoteReqDDFe.ultNSU = ultNSU;
        downloadLoteReqDDFe.modelo = modelo;
        downloadLoteReqDDFe.incluirPDF = incluirPdf;

        if(apenasPendManif){
            downloadLoteReqDDFe.apenasPendManif = true;
        }else{
            downloadLoteReqDDFe.apenasComXml = apenasComXml;
            downloadLoteReqDDFe.comEventos = comEventos;
        }
        String json = objectMapper.writeValueAsString(downloadLoteReqDDFe);
        String url = endpoints.DDFeDownloadLote;
        Genericos.gravarLinhaLog("ddfe", "[DOWNLOAD_LOTE_DADOS]");
        Genericos.gravarLinhaLog("ddfe", json);

        String resposta = NSSuite.enviaConteudoParaAPI(json, url, "json");

        Genericos.gravarLinhaLog("ddfe", "[DOWNLOAD_LOTE_RESPOSTA]");
        Genericos.gravarLinhaLog("ddfe", resposta);

        String retorno = tratamentoDownloadLote(caminho, incluirPdf, resposta);
    ...
    public static String tratamentoDownloadLote(String caminho, boolean incluirPdf, String jsonRetorno) throws IOException {
        JsonNode respostaJSON = objectMapper.readTree(jsonRetorno);
        String status = respostaJSON.get("status").asText();

        if(status.equals("200")){
            ArrayList<String> chRet = salvarDocsLote(caminho, incluirPdf, jsonRetorno);
            DownloadLoteRetDDFe downloadLoteRetDDFe = new DownloadLoteRetDDFe();
            downloadLoteRetDDFe.status = status;
            downloadLoteRetDDFe.ultNSU = respostaJSON.get("ultNSU").asInt();
            downloadLoteRetDDFe.chaves = chRet;
            String json = objectMapper.writeValueAsString(downloadLoteRetDDFe);
            JOptionPane.showMessageDialog(null, "Download em Lote feito com sucesso!");
            return json;
        }else{
            JOptionPane.showMessageDialog(null, respostaJSON.get("motivo").asText());
            return null;
    ...
    public static ArrayList<String> salvarDocsLote(String caminho, boolean incluirPdf, String jsonRetorno) throws IOException {
        ...
        for (int i = 0; i<respostaJSON.size(); i++){
            String xml = respostaJSON.get(i).get("xml").asText();
            if(xml.equals("")){
                continue;
            }
            String tpEvento = "";
            chaves.add(respostaJSON.get(i).get("chave").asText());
            if(respostaJSON.get(i).toString().contains("tpEvento")){
                tpEvento = respostaJSON.get(i).get("tpEvento").asText();
            }else{
                if((incluirPdf) ){
                    String pdf = respostaJSON.get(i).get("pdf").asText();
                    Genericos.salvarPDF(pdf, caminho, chaves.get(i), tpEvento, "");
                    tpEvento = "";
                }
            }
            Genericos.salvarXML(xml, caminho, chaves.get(i), tpEvento, "");
        }
        return chaves;
    }
